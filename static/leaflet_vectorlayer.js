...
L.VectorGrid.Protobuf = L.VectorGrid.extend({
	...
	initialize: function(url, options) {
        ...
        this.on('add', function(e){
            console.log(e);
            self._map && self._map.on("zoomstart", self.cancelPreviousRequests, self);
        });

        this.on('remove', function(e){
            console.log(e);
            self._map && self._map.off("zoomstart", self.cancelPreviousRequests, self);
        });
    },
	...
	getProtobufFetches: function(){
		return this.options.fetches;
	},

	cancelPreviousRequests: function(){
        console.log("cancel");
		var _keys = Object.keys(this.options.fetches);
		for(var i = 0, l = _keys.length; i < l; ++i){
			var _key = _keys[i];
			this.abortTileRequest(_key);
		}
	},

	abortTileRequest: function(fetchKey){
		if(this.options.fetches[fetchKey] != undefined) {
			if(this.options.fetches[fetchKey].abort){
				this.options.fetches[fetchKey].abort();
			}
			delete this.options.fetches[fetchKey];
		}
	},
	...
	_getVectorTilePromise: function(coords) {
		...
		var tileAbort = new AbortController();
		this.options.fetchOptions.signal = tileAbort.signal;

		var _fetchKey = JSON.stringify(coords);
		if(this.options.fetches[_fetchKey] != undefined){
			try{
				this.options.fetches[_fetchKey].abort && this.options.fetches[_fetchKey].abort();
			}catch(e){
				console.log(e);
			}
		}
		this.options.fetches[_fetchKey] = tileAbort;

		var self = this;
		return fetch(tileUrl, this.options.fetchOptions).then(function(response){
			delete self.options.fetches[_fetchKey];
